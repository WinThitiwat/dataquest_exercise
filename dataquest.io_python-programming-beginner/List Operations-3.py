...
weather_data = []
f = open('la_weather.csv').read()
rows = f.split('\n')
for each in rows:
    split_comma = each.split(',')
    weather_data.append(split_comma)
...
weather_type_found = []
for each_weather in weather_types:
    weather_type_found.append(each_weather in new_weather)
